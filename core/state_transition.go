...
func ApplyMessage_new(txExtra *types.TxExtra, evm *vm.EVM, msg *Message, gp *GasPool) (*ExecutionResult, error) {
	return NewStateTransition(evm, msg, gp).TransitionDb_new(txExtra)
}
...
func (st *StateTransition) buyGas_new(txExtra *types.TxExtra) error {
	mgval := new(big.Int).SetUint64(st.msg.GasLimit)
	mgval = mgval.Mul(mgval, st.msg.GasPrice)
	balanceCheck := mgval
	if st.msg.GasFeeCap != nil {
	...
	if have, want := txExtra.GetBalance(st.msg.From), balanceCheck; have.Cmp(want) < 0 {
		return fmt.Errorf("%w: address %v have %v want %v", ErrInsufficientFunds, st.msg.From.Hex(), have, want)
	}
	if err := st.gp.SubGas(st.msg.GasLimit); err != nil {
		return err
	...
	st.initialGas = st.msg.GasLimit
	txExtra.SubBalance(st.msg.From, mgval)
	return nil
}
...
func (st *StateTransition) preCheck_new(txExtra *types.TxExtra) error {
	// Only check transactions that are not fake
	msg := st.msg
	if !msg.SkipAccountChecks {
		// Make sure this transaction's nonce is correct.
		stNonce := txExtra.GetNonce(msg.From)
		if msgNonce := msg.Nonce; stNonce < msgNonce {
			return fmt.Errorf("%w: address %v, tx: %d state: %d", ErrNonceTooHigh,
				msg.From.Hex(), msgNonce, stNonce)
		} else if stNonce > msgNonce {
		...
		// Make sure the sender is an EOA
		codeHash := txExtra.GetCodeHash(msg.From)
		//codeHash := st.state.GetCodeHash(msg.From)
		if codeHash != (common.Hash{}) && codeHash != types.EmptyCodeHash {
			return fmt.Errorf("%w: address %v, codehash: %s", ErrSenderNoEOA,
				msg.From.Hex(), codeHash)
...
func (st *StateTransition) TransitionDb_new(txExtra *types.TxExtra) (*ExecutionResult, error) {
	// First check this message satisfies all consensus rules before
	// applying the message. The rules include these clauses
	//
	// 1. the nonce of the message caller is correct
	...
	// Check clauses 1-3, buy gas if everything is correct
	if err := st.preCheck_new(txExtra); err != nil {
		return nil, err
	}
	...
	// Check clause 6
	//if msg.Value.Sign() > 0 && !st.evm.Context.CanTransfer(st.state, msg.From, msg.Value) {
	if msg.Value.Sign() > 0 && !txExtra.CanTransfer(msg.From, msg.Value) {
		return nil, fmt.Errorf("%w: address %v", ErrInsufficientFundsForTransfer, msg.From.Hex())
	}
...
func (st *StateTransition) refundGas_new(txExtra *types.TxExtra, refundQuotient uint64) {
	...
	remaining := new(big.Int).Mul(new(big.Int).SetUint64(st.gasRemaining), st.msg.GasPrice)
	//st.state.AddBalance(st.msg.From, remaining)
	txExtra.AddBalance(st.msg.From, remaining)
